...
| 지표 | print 있음 (doPrint) | print 없음 (dontPrint) |
|------|---------------------|----------------------|
| 평균 응답 시간 | 직접 측정해보자 | 직접 측정해보자 |
| p95 응답 시간 | 직접 측정해보자 | 직접 측정해보자 |
| 초당 처리량 (req/s) | 직접 측정해보자 | 직접 측정해보자 |
...
참고로, 내 로컬 환경(M1 MacBook, Python 3.12)에서 측정했을 때 doPrint의 평균 응답 시간은 약 300~500ms, dontPrint는 약 3~8ms 수준이었다. 환경에 따라 수치는 다르겠지만, 수십 배에서 수백 배 차이가 나는 방향성은 동일하다.

놀랍지 않은가? 코드 로직은 똑같은데, `print()` 유무만으로 이 정도 차이가 난다. 왜 이런 일이 벌어지는지 코드부터 보자.
